...
from tools import finetune_run_net_hardest as finetune_hardest
...
def main():
    ...
    if args.test:
        ...
        if args.finetune_model or args.scratch_model:
            test_base = str(config.dataset.test._base_)
            if 'hardest' in os.path.basename(test_base) and config.model.type == "full":
                finetune_hardest(args, config, train_writer, val_writer)
            else:
