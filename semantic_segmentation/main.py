...
## we use adamw and cosine scheduler
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            mIoU = np.mean(np.array(total_correct_class) / (np.array(total_iou_deno_class, dtype=float) + 1e-6))    # np.float -> float
            mAcc = np.mean(np.array(total_correct_class) / (np.array(total_seen_class, dtype=float) + 1e-6)) 
            ...
            log_string('[mAcc] eval point avg class acc: %f' % (mAcc * 100.0))

            iou_per_class_str = '------- IoU --------\n'
            for l in range(NUM_CLASSES):
                iou_per_class_str += 'class %s weight: %.3f, Acc: %.3f \n' % (
                    seg_label_to_cat[l] + ' ' * (14 - len(seg_label_to_cat[l])), labelweights[l - 1],
                    total_correct_class[l] / float(total_seen_class[l]) * 100.0)
            ...
            if mAcc >= best_mACC:
                best_mACC = mAcc
                logger.info('Save model...')
                savepath = str(checkpoints_dir) + '/best_mACC_model.pth'
                log_string('Saving at %s' % savepath)
                state = {
                    'epoch': epoch,
                    'class_avg_iou': mIoU,
                ...
                torch.save(state, savepath)
                log_string('Saving model....')
            log_string('>>> Epoch %03d ETA: %s Best mIoU: %f Best mAcc: %f' % (global_epoch + 1, eta_str, best_iou * 100.0, best_mACC*100.0))
            global_epoch += 1
