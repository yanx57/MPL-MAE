...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        losses = AverageMeter(['Loss', 'loss2', 'loss3'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            loss1, loss2, loss3 = base_model(points)
            ...
            if args.distributed:
                loss1 = dist_utils.reduce_tensor(loss1, args)
                #loss2 = dist_utils.reduce_tensor(loss2, args)   # maybe bug here (DDP has not been tested)
                losses.update([loss1.item()*1000])
            else:
                losses.update([loss1.item()*1000, loss2.item()*1000, loss3.item()*1000])
