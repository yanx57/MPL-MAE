...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # optimizer & scheduler
    optimizer, scheduler = builder.build_opti_sche_ft(base_model, config)
    ...
    ema_model = ema.EMA(model=base_model, decay=config.model.ema_decay)
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            metrics = validate(ema_model.shadow, test_dataloader, epoch, val_writer, args, config, best_metrics, logger=logger)     
            better = metrics.better_than(best_metrics)
            # Save ckeckpoints
            if better:
                best_metrics = metrics
                metric_path = os.path.join(args.exp_name, "best_metric.txt")
                with open(metric_path, "w") as f:
                    f.write(f"best_acc = {best_metrics.acc:.4f}\n")
                if metrics.acc>90.2:
                    # builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, 'ckpt-best', args, logger = logger)
                    builder.save_checkpoint(ema_model.shadow, optimizer, epoch, metrics, best_metrics, 'ckpt-best', args, logger = logger)
                print_log("ema--------------------------------------------------------------------------------------------", logger=logger)
            # metrics = validate(base_model, test_dataloader, epoch, val_writer, args, config, best_metrics, logger=logger)     
            # better = metrics.better_than(best_metrics)
            # # Save ckeckpoints
            # if better:
            ...
            #     if metrics.acc>90.2:
            #         builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, 'ckpt-best', args, logger = logger)
            #     print_log("base--------------------------------------------------------------------------------------------", logger=logger)
            if args.vote:
                # metrics_vote = validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, best_metrics_vote, logger=logger)
    ...
    def cleanup():
    # 1. 释放模型和优化器占用的显存对象
        global model, optimizer # 或者传入你的变量名
        try:
            del model
            del optimizer
        except:
        ...
        # 3. 清空 PyTorch 内部的显存缓存池
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            torch.cuda.synchronize() # 阻塞直到所有核函数执行完毕
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, best_metrics_vote, logger = None, times = 10):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            assert points_raw.size(1) < point_all
            ...
            local_pred = []
            voting_angles = [i * (2 * np.pi / 10) for i in range(times)]
            for kk in range(times):
                ...
                points = pointnet2_utils.gather_operation(points_raw.transpose(1, 2).contiguous(), 
                                                        fps_idx_raw).transpose(1, 2).contiguous()  # (B, N, 3)
                
                points = data_transforms.rotate_point_cloud_by_angle(points, voting_angles[kk])

                logits = base_model(points)
                probs = torch.softmax(logits, dim=-1) # 转化为概率
                local_pred.append(probs.detach().unsqueeze(0)) # 存储概率分布
                target = label.view(-1)
        ...
        acc = (test_pred == test_label).sum() / float(test_label.size(0)) * 100.
        print_log('[Validation_vote] EPOCH: %d  acc_vote = %.4f best_vote = %.4f' % (epoch, acc, max(best_metrics_vote.acc, acc)), logger=logger)
